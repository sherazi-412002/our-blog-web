...
function Header() {
  ...
  return (
    <div>
       ...
       <header className='flex justify-between items-center h-20 p-1 bg-gradient-to-r from-[#00AAA1] to-yellow-600 text-white left-0 right-0 top-0 w-[100%]  mt-2 fixed'>
        <div>
            <Image src={logo} alt='Logo' width={500} height={500} className=' h-16  w-16 ml-2 sm:m-16 md:ml-14 lg:ml-24 rounded-full'/>
             
        </div>

        <div>
            <ul className=' hidden md:flex justify-between gap-14 text-2xl font-bold mr-20'>
                <Link href={'/'}><li className=' hover:rounded-lg hover:opacity-75 hover:scale-100 active:scale-95 p-1'>Home</li></Link>
        ...
        {isClick &&  (
            ...
            <ul className='flex flex-col  text-center pr-4 gap-3 pb-6 pt-6  bg-white text-2xl hover:duration-300 hover:delay-300'>
